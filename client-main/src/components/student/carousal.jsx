...
const slides = [
  {
    ...
    image: assets.carousal1, // Updated to use carousal1
  },
  {
    ...
    image: assets.carousal2, // Updated to use carousal2
  },
  {
    ...
    image: assets.carousal3, // Updated to use carousal3
  },
];
    ...
    <div className="relative w-screen overflow-hidden rounded-none shadow-lg bg-gradient-to-r from-gray-50 to-gray-100">
      ...
      >
        {slides.map((slide, index) => (
          <div
            key={index}
            className="flex-none w-full flex h-[450px] bg-white"
          >
            {/* Left Image */}
            <div className="w-1/2 flex items-center justify-center bg-gray-100 rounded-r-[100px] overflow-hidden">
              <img
                src={slide.image}
                alt={slide.heading}
                className="w-4/5 h-4/5 object-cover rounded-2xl shadow-lg hover:scale-105 transition-transform duration-500"
              />
            </div>
            {/* Right Content */}
      ...
      <button
        onClick={prevSlide}
        className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-white bg-opacity-50 p-3 rounded-full shadow-lg hover:bg-opacity-70 hover:scale-110 transition-transform duration-300"
      >
        <ChevronLeft size={24} />
      </button>
      <button
        onClick={nextSlide}
        className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-white bg-opacity-50 p-3 rounded-full shadow-lg hover:bg-opacity-70 hover:scale-110 transition-transform duration-300"
      >
